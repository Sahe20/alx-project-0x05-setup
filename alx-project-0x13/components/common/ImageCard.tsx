...
const ImageCard: React.FC<GeneratedImageProps> = ({ imageUrl, prompt, width, action }) => {
  return (
    <div onClick={() => action(imageUrl)} className="mt-6 border hover:cursor-pointer p-2 rounded-lg shadow-md bg-white">
      <img
        src={imageUrl}
        alt={prompt}
        className={`w-full max-w-md rounded-lg shadow-lg ${width ? width : ""}`}
      />
